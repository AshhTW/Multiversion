...
class LatestChunkSerializerWrapper implements MVChunkSerializer{
	 ...
	 */
	public function serializeFullChunk(Chunk $chunk, int $dimensionId, MVBlockTranslator $blockTranslator, PacketSerializerFactory $factory, ?string $tiles = null) : string{
		...
		for($y = Chunk::MIN_SUBCHUNK_INDEX; $y <= Chunk::MAX_SUBCHUNK_INDEX; ++$y){
			ReflectionUtils::invokeStatic(
				...
				"serializeBiomePalette",
				$chunk->getSubChunk($y)->getBiomeArray(),
				$biomeIdMap,
				$this->getBiomePaletteSerializerArg($stream)
			);
		}
	...
	/**
	 * PMMP changed the 3rd arg from PacketSerializer to ByteBufferWriter.
	 * We select the correct stream object at runtime to keep compatibility.
	 */
	private function getBiomePaletteSerializerArg(PacketSerializer $stream) : PacketSerializer|ByteBufferWriter{
		$method = new \ReflectionMethod(ChunkSerializer::class, "serializeBiomePalette");
		$type = $method->getParameters()[2]?->getType();
		if($type instanceof \ReflectionNamedType && $type->getName() === ByteBufferWriter::class){
	...
	public function serializeSubChunk(SubChunk $subChunk, MVBlockTranslator $blockTranslator, PacketSerializer $stream, bool $persistentBlockStates) : void{
		...
		foreach($layers as $blocks){
			...
			if($persistentBlockStates){
				...
				foreach($palette as $p){
					$stream->putUnsignedVarInt($blockTranslator->internalIdToNetworkId($p) << 1);
				}
			}
		}
	}
	...
	public function serializeTiles(Chunk $chunk) : string{
		...
		if(self::$serializeTilesExpectsTypeConverter){
			return ChunkSerializer::serializeTiles($chunk, TypeConverter::getInstance());
		}
