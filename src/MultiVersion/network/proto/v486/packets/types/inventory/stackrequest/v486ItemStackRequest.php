...
final class v486ItemStackRequest{
	 ...
	 */
		private static function readAction(PacketSerializer $serializer, ByteBufferReader $reader, int $typeId) : ItemStackRequestAction{
		$action = match ($typeId) {
			...
			CraftingCreateSpecificResultStackRequestAction::ID => CraftingCreateSpecificResultStackRequestAction::read($reader),
			LabTableCombineStackRequestAction::ID => LabTableCombineStackRequestAction::read($reader),
			BeaconPaymentStackRequestAction::ID => BeaconPaymentStackRequestAction::read($reader),
			MineBlockStackRequestAction::ID => MineBlockStackRequestAction::read($reader),
			...
			CraftRecipeOptionalStackRequestAction::ID => CraftRecipeOptionalStackRequestAction::read($reader),
			v486GrindstoneStackRequestAction::ID => v486GrindstoneStackRequestAction::read($serializer),
			LoomStackRequestAction::ID => LoomStackRequestAction::read($reader),
			DeprecatedCraftingNonImplementedStackRequestAction::ID => DeprecatedCraftingNonImplementedStackRequestAction::read($reader),
			DeprecatedCraftingResultsStackRequestAction::ID => DeprecatedCraftingResultsStackRequestAction::read($reader),
	...
	public static function read(ByteBufferReader|PacketSerializer $in) : self{
		...
		for($i = 0, $len = $serializer->getUnsignedVarInt(); $i < $len; ++$i){
			...
			$actions[] = self::readAction($serializer, $reader, $typeId);
		}
		$filterStrings = [];
