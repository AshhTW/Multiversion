...
final class v419ItemStackRequest{
	 ...
	 */
	private static function readAction(PacketSerializer $serializer, ByteBufferReader $reader, int $legacyTypeId) : ItemStackRequestAction{
		$action = match($legacyTypeId){
			...
			self::LEGACY_CRAFTING_MARK_SECONDARY_RESULT_SLOT => CraftingCreateSpecificResultStackRequestAction::read($reader),
			self::LEGACY_LAB_TABLE_COMBINE => LabTableCombineStackRequestAction::read($reader),
			self::LEGACY_BEACON_PAYMENT => BeaconPaymentStackRequestAction::read($reader),
			...
			self::LEGACY_CRAFTING_NON_IMPLEMENTED_DEPRECATED => DeprecatedCraftingNonImplementedStackRequestAction::read($reader),
	...
	public static function read(ByteBufferReader|PacketSerializer $in) : self{
		...
		for($i = 0, $len = $serializer->getUnsignedVarInt(); $i < $len; ++$i){
			$actions[] = self::readAction($serializer, $reader, $serializer->getByte());
		}
	 ...
	 * @throws ReflectionException
	 */
	private static function writeAction(PacketSerializer $serializer, ItemStackRequestAction $action) : void{
		...
		if($action instanceof v419TakeStackRequestAction ||
			...
			$action->write($serializer);
		}elseif($action instanceof CraftRecipeAutoStackRequestAction){
			(new v419CraftRecipeAutoStackRequestAction($action->getRecipeId()))->write($serializer);
		}elseif($action instanceof CraftRecipeStackRequestAction){
			(new v419CraftRecipeStackRequestAction($action->getRecipeId()))->write($serializer);
		}elseif($action instanceof CreativeCreateStackRequestAction){
			(new v419CreativeCreateStackRequestAction($action->getCreativeItemId()))->write($serializer);
		}elseif($action instanceof DeprecatedCraftingResultsStackRequestAction){
			(new v419DeprecatedCraftingResultsStackRequestAction($action->getResults(), $action->getIterations()))->write($serializer);
		}else{
			$action->write($serializer->getWriter());
		}
	}
	 ...
	 * @throws ReflectionException
	 */
	public function write(ByteBufferWriter|PacketSerializer $out) : void{
		...
		foreach($this->actions as $action){
			...
			self::writeAction($serializer, $action);
		}
	}
}
